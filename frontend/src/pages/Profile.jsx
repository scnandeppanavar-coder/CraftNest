...
const Profile = () => {
  ...
  const handleModalChangePassword = async (e) => {
    ...
    if (!confirmPassword.trim()) {
      showToast('Confirm New Password is required', 'warning');
      return;
    }
    ...
    try {
      ...
      console.error(error);
      const errMsg = error.response?.data?.message || 'Failed to change password. Please verify current password.';
      showToast(errMsg, 'error');
    } finally {
  ...
  const handlePhotoUpload = (e) => {
    ...
    if (!validTypes.includes(file.type)) {
      showToast('Invalid file type. Only JPG, JPEG, and PNG are allowed.', 'error');
      return;
    }
  ...
  return (
    <div className="space-y-8 pb-16">
      {/* Header Banner */}
      <div className="pb-4 border-b border-secondary-100 dark:border-secondary-800 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 border border-[#D67A57] hover:bg-[#D67A57]/10 text-[#D67A57] font-bold text-xs rounded-xl active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
          >
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-white dark:bg-secondary-900 border border-secondary-200/50 dark:border-secondary-800 p-6 rounded-3xl text-center space-y-4 shadow-sm transition-colors">
            <div className="relative mx-auto w-24 h-24">
              <div className="w-24 h-24 rounded-full bg-secondary-100 dark:bg-secondary-800 text-secondary-500 dark:text-secondary-400 flex items-center justify-center border-4 border-white dark:border-secondary-900 shadow-sm overflow-hidden">
                {user?.profilePic ? (
              ...
              <label htmlFor="avatar-upload-sidebar" className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-[#D67A57] hover:bg-[#c96e4c] text-white flex items-center justify-center shadow-md cursor-pointer border-2 border-white dark:border-secondary-900 transition-all hover:scale-105 active:scale-95">
                ...
                <div className="bg-white dark:bg-secondary-900 border border-secondary-200/50 dark:border-secondary-800 p-6 sm:p-8 rounded-3xl shadow-sm transition-colors space-y-6">
                  ...
                  <form onSubmit={handleUpdateDetails} className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          disabled
                          value={addresses.find(a => a.isDefault)?.phone || addresses[0]?.phone || 'Not available'}
                          className="w-full px-4 py-3 bg-secondary-100/50 dark:bg-secondary-900/50 border border-secondary-200 dark:border-secondary-800 rounded-2xl text-xs focus:outline-none dark:text-secondary-400 transition-all opacity-80"
                        />
                      </div>

                      {/* Member Since */}
                      <div>
                        <label className="text-xs font-semibold text-secondary-500 dark:text-secondary-405 block mb-1.5 ml-1">
                          Member Since
                        </label>
                        <input
                          type="text"
                          disabled
                          value={user?.createdAt ? new Date(user.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' }) : 'August 2026'}
                          className="w-full px-4 py-3 bg-secondary-100/50 dark:bg-secondary-900/50 border border-secondary-200 dark:border-secondary-800 rounded-2xl text-xs focus:outline-none dark:text-secondary-400 transition-all opacity-80"
                        />
                      </div>
                    </div>
                    ...
                    <div className="border-t border-secondary-100 dark:border-secondary-800 pt-6">
                      <label className="text-xs font-semibold text-secondary-500 dark:text-secondary-400 block mb-3 ml-1">
                        Profile Picture
                      </label>
                      <div className="flex flex-col sm:flex-row items-center gap-4 bg-secondary-50/50 dark:bg-secondary-800/20 p-4 rounded-2xl border border-secondary-200/50 dark:border-secondary-800">
                        <div className="w-14 h-14 rounded-full bg-secondary-100 dark:bg-secondary-800 text-secondary-500 dark:text-secondary-400 flex items-center justify-center border border-secondary-200 dark:border-secondary-750 overflow-hidden shrink-0">
                          {user?.profilePic ? (
                            <img src={user.profilePic} alt="Profile Preview" className="w-full h-full object-cover" />
                          ) : (
                        ...
                        <div className="flex-1 text-center sm:text-left space-y-1">
                          ...
                          <p className="text-[10px] text-secondary-450 dark:text-secondary-400">
                        ...
                        >
                          ...
                          <div className="shrink-0 flex flex-col sm:flex-row items-center gap-2">
                            <Link
                              ...
                              className="w-full sm:w-auto px-4 py-2 bg-white dark:bg-secondary-900 hover:bg-secondary-100 dark:hover:bg-secondary-850 text-secondary-800 dark:text-white border border-secondary-200 dark:border-secondary-750 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-sm"
                            >
                              <Eye className="w-3.5 h-3.5 text-secondary-500" /> Details
                            </Link>
                            <Link
                              to={`/invoice/${order.orderId}`}
                              className="w-full sm:w-auto px-4 py-2 bg-[#D67A57]/10 hover:bg-[#D67A57]/20 text-[#B8633F] dark:text-[#F3C8AF] font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-sm cursor-pointer"
                            >
                              Download Invoice
                            </Link>
                          </div>
      ...
      {changePasswordModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-[#FAF7F2] dark:bg-secondary-900 border border-secondary-200/50 dark:border-secondary-800 w-full max-w-md p-6 sm:p-8 rounded-[32px] shadow-2xl relative space-y-6 animate-scale-in">
            {/* Close Button X */}
            <button
              onClick={() => {
                setChangePasswordModalOpen(false);
              ...
              className="absolute top-4 right-4 text-secondary-400 hover:text-secondary-600 dark:hover:text-secondary-200 transition-colors cursor-pointer"
            >
            ...
            <div className="text-center">
              <h3 className="font-outfit font-black text-xl text-secondary-900 dark:text-white uppercase tracking-wider">
                Change Password
              </h3>
              <p className="text-[10px] text-secondary-450 dark:text-secondary-400 mt-1 font-bold uppercase tracking-widest">
                Update account security settings
              </p>
            </div>

            <form onSubmit={handleModalChangePassword} className="space-y-4">
              {/* Current Password */}
              <div>
                <label className="text-xs font-semibold text-secondary-500 dark:text-secondary-400 block mb-1.5 ml-1">
                  Current Password
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 bg-white dark:bg-secondary-950 border border-secondary-200 dark:border-secondary-800 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 rounded-2xl text-xs focus:outline-none dark:text-white transition-all"
                  />
              ...
              <div>
                ...
                <span className="text-[10px] font-medium text-secondary-450 dark:text-secondary-400 mt-1.5 block ml-1">
              ...
              <div className="flex items-center justify-end gap-3 mt-6">
                <button
                  ...
                  className="px-5 py-2.5 bg-secondary-100 hover:bg-secondary-200 dark:bg-secondary-800 dark:hover:bg-secondary-750 text-secondary-850 dark:text-white font-bold text-xs rounded-xl active:scale-95 transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-[#D67A57] hover:bg-[#c96e4c] text-white font-bold text-xs rounded-xl active:scale-95 transition-all flex items-center gap-1.5 cursor-pointer shadow-md shadow-primary-500/10"
                >
                  {changingPassword ? (
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  ) : (
